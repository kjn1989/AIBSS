# AI-BSS公式クラウド セットアップ手順(運営者向け・Supabase版)

AI-BSS公式クラウドは、運営者が1つ持つSupabaseプロジェクトを全チームで共用する仕組みです。
...
1. アプリの設定タブ → 「☁️ AI-BSS公式クラウド」がログインUIに変わる
...
- **休眠**: 無料プランは1週間アクセスが無いとプロジェクトが一時停止する
  (ダッシュボードの「Restore」で再開。日常利用があれば停止しない)
