...
//   大きさ … 読めたとき、勝率がどれだけ動いたか   (分母 = 読めた回数)
// 「押した11回 → 読めた3回 → そのとき平均31ポイント動いた」と鎖でつながるので、
// どちらを見ればいいのか迷わない。
//
// 昔の2つ(精度と広さ)が読めなかったのは、分母が「押した数」と「試合で起きた動き
// 全部」で、競合する2つの物差しになっていたため。しかも後者は「感じたときだけで
// OK」と書いてあるタグを、押さないと下がる形で罰していた。
// 勝率ポイントの合計を成績にする形も試したが、天井も比べる先も無く読めなかった。
// ------------------------------------------------------------
export function judgeFlowTags(game, series = [], opts = {}) {
  ...
  // 読めた回で、勝率がその向きへ動いた幅の合計(0〜1の単位)。
  // 読めなかった回の gain は0なので、実質「読めた回だけの和」になる。
  let points = 0;
  for (const tg of tags) points += moves[tg.id]?.gain || 0;
  return {
    tags, verdict, counts: c, swings, moves,
    // 確度: 押したうち、動く前に読めていた割合(分母は押した回数)
    hitRate: tags.length ? c.pre / tags.length : null,
    // 大きさ: 読めたとき、勝率がどれだけその向きへ動いたか(分母は読めた回数)
    // 押した回数で割ってはいけない。読めなかった回に薄められて、
    // 「どれくらい当たるか」でも「当たるとどれくらい大きいか」でもない数になる。
    points,
    avgMove: c.pre ? points / c.pre : null,
