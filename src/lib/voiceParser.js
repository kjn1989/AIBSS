...
// (ひらがな化した後に適用)
const ASR_FIXES = [
  [/まいひっと|舞ひっと|まいひと/g, '前ひっと'], // 「前ヒット」の誤認識「マイヒット/舞ヒット」を補正(単打シノニム'前ヒット'に一致)
  [/まえひっと/g, '前ひっと'], // 「前ヒット」がかな書きで来た場合も同様に揃える
  [/金振り|金振|かなぶり|かなぶん|かなぶ/g, '空振り'], // 「空振り」の誤認識「金振り/かなぶり」を補正
...
// 確認カードに出す「聞き取り結果」表示用の軽い整形(誤認識を自然な表記に戻す)。
// 解釈には影響せず、見た目だけ整える。
const DISPLAY_FIXES = [
  [/マイヒット/g, '前ヒット'],
  [/まいひっと/g, '前ヒット'],
...
export function prettifyTranscript(raw) {
  let t = raw || '';
  for (const [re, to] of DISPLAY_FIXES) t = t.replace(re, to);
  return t;
}
...
const PITCH_DICT = {
  ...
  // 長語で優先され、下の投球ループのガードでstrikeは抑制される
  strike: ['ストライク', '見逃し', '空振り', '空振', 'からぶり', 'からぶ', 'すとらいく'],
  foul: ['ファウル', 'ファール', 'ふぁうる'],
};
