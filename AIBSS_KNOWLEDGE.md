...
### 第2層: 英語画面に日本語が混ざる(データ層に言語が無い)

- **守備位置**: `POSITIONS` が生の日本語1文字配列で、`holes.join('・')` が英文に入る。
  実物 → `⚠️ No registered player can cover 投・捕・一・二・三・遊・左・中・右.`
  **`positionLabel(pos, lang)` は既にあるのに呼ばれていないだけ**(`HeadCoachView.jsx:108/126`、`SettingsTab.jsx:698`)
- **試合結果のハイライト**: `highlights.js` が `lang` を受け取らない。`h.clutch.label`(「8回裏 松本 犠打」)、
  `h.topPitcher.tag`(「勝利投手」)、`h.topPitcher.line`(「5.0回 7奪三振 自責1」)が素通し
- **K/BB**: `stats.js:333` が `${s.strikeouts} (与四球0)` を直接組み立てている
- **設定ギアの `aria-label`** が「設定」固定(`App.jsx:186`)
- **CSV**(出力・取り込みテンプレート)のヘッダと注記が全て日本語

### 第3層: 機能そのものが日本語専用

- **音声**: `speech.js` が `rec.lang = 'ja-JP'` 固定、`tts.js` も同様。`voiceParser.js`(96行)と
  `correctionParser.js`(68行)は日本語の言い回し専用のルールベース解析器。
  **フォールバックのテキスト入力UIまで日本語**
- **AIの出力**: `gemini.js` のプロンプトが全て日本語で、出力言語の指定が無い。
  英語モードでも日本語の記事が返る。**プロンプトに言語指定を足すだけなので、ここは安い**
...
第2層は既存の変換関数を呼ぶだけのものが多く、数時間で「英語モードが英語で完結」する。
