...
- **フィリピン代表チームでの利用が検討されている**。**英語対応は完了した**(§9)。
  残っているのは音声認識エンジンの英語版だけで、これは翻訳ではなく新規開発。
  英語でもテキスト入力で全機能が使える。
...
| 画面 | 対応 |
|---|---|
| `NewspaperView.jsx`(AIスポーツ新聞) | 画面 + テンプレート記事 + AIへ渡すサマリー + 新聞画像(Canvas)の題字・日付行・キャプション・フッター |
| `ScoutCard.jsx`(AI選手名鑑) | 画面 + 寸評タグ約60語 + キャッチフレーズ + ダミー寸評 |
| `HeadCoachView.jsx`(AIヘッドコーチ) | 画面 + 成績サマリー + エラー文 |
| `VoiceControl.jsx`(音声入力) | 画面 + 読み上げ文。**認識エンジン自体は日本語のまま**(後述) |

**AI選手名鑑のタグは、訳すだけでは壊れる。** タグは画面の文字ではなく
**保存されるデータ**で、選手に `{ label, type }` で載る。単純に訳すと、日本語で
付けたタグを英語表示で見たとき、保存済みのラベル(日本語)と候補ボタンのラベル(英語)が
別物になり、**同じタグを二重に付けられてしまう**。

プリセットに言語に依らない `id` を持たせ、表示だけ id から引く。同一判定も id を
優先する。id を持たない古いデータは `JA_LABEL_TO_ID` で日本語ラベルから逆引きして
表示だけ英語にする。**データは書き換えない**(書き換えると、同期で古い端末と
新しい端末のあいだを往復して壊れる)。

寸評タグは直訳せず英語圏の野球の言い方に置き換えた
(「レーザービーム」→ Cannon arm、「ポテンヒット製造機」→ Gives up bloopers)。

**文のつなぎ方も言語で違う。** 新聞のテンプレート記事も名鑑のダミー寸評も、短い文を
つないで1本にしている。日本語は続けて書けるが英語は文のあいだに空白が要るので、
つなぎ文字自体を辞書に持たせた(`np.fbJoin` / `scout.sentenceJoin`)。
...
- **音声 — UIは対応済み / 認識エンジンは日本語のまま**: `voiceParser.js`(96行)と
  `correctionParser.js`(68行)は日本語の言い回し専用のルールベース解析器で、これを英語向けに
  作るのは翻訳ではなく新規開発なので手を付けていない。
  一方で**UI・読み上げ・フォールバックのテキスト入力は英語化した**。
  読み上げは `setSpeakLang()` で表示言語に合わせる(英語の文を `ja-JP` の声で読ませると
  単語ごとにローマ字読みになって聞き取れない)。
  英語表示で音声シートを開くと「認識は日本語の実況のみ。テキスト入力を使ってください」と
  先に出す(`vc.jaOnly`)。押してから無反応になるのが一番たちが悪いため

### 第4層: ガワ — ✅ 対応済み

`index.html` は静的なので、何もしないと日本語のまま英語のユーザーに配られる。
`src/lib/documentLang.js` に集約し、**起動時(`main.jsx`)と言語を切り替えた瞬間(`App.jsx`)の
両方**から呼ぶ(起動時だけだと、切り替えても次に開くまでタブの名前が古いままになる)。

- `<html lang>` … スクリーンリーダーの読み上げとブラウザの翻訳提案が見るところ
- `<title>` … タブの名前、共有したときのリンク名
- **manifest** … ホーム画面に追加したときのアプリ名。静的ファイルなので
...
英語対応は「一度やって終わり」にならない。日本語だけ足して英語を忘れると静かに戻るので、
**3段の機械チェック**を入れてある。どれも一度わざと壊して落ちることを確認済み。

1. **`scripts/check-i18n.mjs`**
   - ja/en のキー整合(以前からある)
   - **キーの二重定義**。JSのオブジェクトリテラルは後の定義が勝つので、二重定義は
     実行時に見えずキー整合も通る。実際に起きたのは AI選手名鑑の `sc.title` が
     記録員カードの `sc.title` と衝突して、名鑑のヘッダーに「記録員の読み」と出た事故
     (名鑑側を `scout.` に切り出して解決)
   - **ソースが使うキーが辞書に実在するか**。未定義キーは `translate()` が
     **キー名をそのまま返す**ので、画面に `scout.title` と出るだけで例外にも
     日本語検査にも掛からない
2. **`tests/unit.mjs`** — 英語辞書に日本語の文字が残っていないか。
   `t(\`tag.${id}\`)` のような動的キーは静的走査で拾えないので、一覧を持って両言語そろっているかを見る
3. **`tests/english.e2e.mjs`** — 実際に英語へ切り替えて全タブ+選手ページ+AI選手名鑑を開き、
   描画テキスト・placeholder・aria-label・title を集めて日本語とキー名の素出しを弾く。
   静的検査では「辞書を通さず直接書いた日本語」が見えないため、実物を見る側も要る

⚠️ **訳文で分岐しないこと。** 勝敗の色分けを `resultLabel === '勝利'` で判定していたため、
英語にすると共有画像の勝敗ピルが全部グレーになるところだった。同じく
「チーム内1位」の材料は訳文に「タイ」が入っているかで単独首位を見分けていた。
どちらも `resultKey` / `{ text, tied }` のようにデータ側へ印を移してある。
